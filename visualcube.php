...
require 'assets/visualcube/visualcube_config.php';
...
if(!array_key_exists('fmt', $_REQUEST)){
	...
	include 'assets/visualcube/visualcube_api.php';
		...
		$ABBR_COL = Array(
			...
			if(!$uf){ $uf = array_key_exists('fd', $_REQUEST) ? $_REQUEST['fd'] : $DEFAULTS['fd'];
				...
				if(array_key_exists('stage', $_REQUEST) || array_key_exists('stage', $DEFAULTS)){
					...
					if($mask && $st_rtn != ''){
						require_once "assets/visualcube/cube_lib.php";
				...
				/** Alternative version using files rather than pipes, not desired because of collision possibilities.. */
